...
sayHelloButton.addEventListener(`mouseover`, helloButtonMouseover);
...
sayHelloButton.addEventListener(`mouseout`, helloButtonMouseout);
...
const queryRequest = () => {
    return axios.get(`http://localhost:3000/query-test?any-query=query-one&one-more-query=query-two`)
    .then(res => {
   ...
   .then(res => {
    ...
    foodInput.value =``
       for (let i = 0; i< res.data.length; i++){
           ...
           foodText.textContent = res.data[i]
           listSpace.appendChild(foodText)
